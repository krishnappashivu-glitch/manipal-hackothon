      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between gap-6">
            ...
            <div className="flex gap-4 text-center">
                 <div>
                    <div className="text-slate-500 text-xs font-bold uppercase">Confidence</div>
                    <div className="text-xl font-bold text-white">{(selectedWallet.confidenceScore * 100).toFixed(0)}%</div>
                 </div>
                 <div>
                    <div className="text-slate-500 text-xs font-bold uppercase">Score</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8 relative z-10">
          <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-800">
            ...
            <div className="text-lg font-bold text-slate-200">{selectedWallet.totalSent.toLocaleString()}</div>
          </div>
          <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-800">
             <div className="text-slate-500 text-xs uppercase font-bold mb-1">Inbound Flow</div>
            <div className="text-lg font-bold text-slate-200">{selectedWallet.totalReceived.toLocaleString()}</div>
          </div>
          <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-800">
        ...
        <div className="md:col-span-2 space-y-6">
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
                    <Bot size={18} className="text-cyan-400" /> Agent Report
                </h3>
                ...
                <div className="mt-6 pt-6 border-t border-slate-800 space-y-3">
                    <h4 className="text-sm font-semibold text-slate-400">Behavioral Flags</h4>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 h-fit">
          ...
          <div className="overflow-y-auto max-h-[500px] pr-2 space-y-2">
              {selectedWallet.transactions.map((tx) => {
                  ...
                  return (
                      <div key={tx.tx_id} className="p-3 bg-slate-950 rounded border border-slate-800 text-xs flex justify-between items-center hover:border-slate-700 transition-colors cursor-default">
                          <div className="flex flex-col gap-1">
                              <span className="text-slate-500 font-mono text-[10px]">
                                {tx.timestamp.includes('T') ? tx.timestamp.split('T')[1].replace('Z','') : tx.timestamp}
                              </span>
                              <span className="font-mono text-slate-300 truncate max-w-[100px]" title={isIncoming ? tx.from_wallet : tx.to_wallet}>
