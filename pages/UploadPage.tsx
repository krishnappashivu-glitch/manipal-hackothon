  ...
  return (
    <div className="max-w-4xl mx-auto py-12">
      <div className="text-center mb-12">
        <h2 className="text-4xl font-bold text-white mb-4">ChainSleuth AI <span className="text-cyan-400">Forensics</span></h2>
      ...
      <div className="flex justify-center mb-8">
        <div className="bg-slate-900 p-1 rounded-xl border border-slate-800 flex gap-1">
            <button 
                ...
                className={`px-6 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'upload' ? 'bg-slate-800 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}
            >
                <FileText size={16} /> Historical Analysis (CSV)
            </button>
            <button 
                onClick={() => setActiveTab('live')}
                className={`px-6 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${activeTab === 'live' ? 'bg-indigo-900/50 text-indigo-300 border border-indigo-500/30' : 'text-slate-400 hover:text-slate-200'}`}
            >
      ...
      <div className="bg-slate-950 border border-slate-800 rounded-2xl p-8 min-h-[400px] flex flex-col justify-center">
        {activeTab === 'upload' ? (
            <div className="space-y-8">
                <div className="border-2 border-dashed border-slate-700 rounded-2xl p-12 text-center hover:border-cyan-500/50 transition-colors group relative overflow-hidden bg-slate-900/50">
                    <input
                    type="file"
                    accept=".csv"
                    ...
                    <div className="w-16 h-16 bg-slate-800 rounded-full flex items-center justify-center group-hover:bg-slate-700 transition-colors">
                        {isProcessing ? <Activity className="animate-spin text-cyan-400" /> : <Upload className="text-cyan-400" size={32} />}
                    </div>
                    <div>
            ...
            <div className="text-center space-y-6">
                <div className="w-20 h-20 bg-indigo-900/20 rounded-full flex items-center justify-center mx-auto border border-indigo-500/20">
                    <Blocks className="text-indigo-400" size={40} />
                </div>
                <div>
                ...
                <div className="bg-slate-900 max-w-md mx-auto rounded-lg p-4 text-left border border-slate-800 space-y-3">
                    ...
                    <div className="flex items-center gap-3 text-sm text-slate-500">
                        <div className="w-4 h-4 rounded-full border border-slate-600" /> Bitcoin (Coming Soon)
                    </div>
                </div>
                ...
                <button 
                    ...
                    disabled={isProcessing}
                    className="px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-bold shadow-lg shadow-indigo-500/20 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      <div className="mt-8 flex gap-4 justify-center text-xs text-slate-500">
